...
db = client['services']
pcollection = db.get_collection('logintimedetails')
records = pcollection.find({})
timevalues = []
for rec in records:
    timevalues.append(rec['logintime'])
numpytimevalues = np.array(timevalues)
###################################################
std_deviation = statistics.stdev(timevalues)
print(std_deviation)
mean = statistics.mean(timevalues)
print(mean)
datalenghti = len(timevalues)
print(len(timevalues))
###################################################
## reference: https://matplotlib.org/3.1.0/gallery/statistics/histogram_cumulative.html
#https://matplotlib.org/examples/statistics/histogram_demo_cumulative.html
#np.random.seed(0)

mu = mean #200
sigma = std_deviation #25
n_bins = datalenghti #50
#x = np.random.normal(mu, sigma, size=100)#mean and standard deviation
x = numpytimevalues
print(numpytimevalues)
...
ax.set_xlabel('Time for Connecting a Service (seconds)')
