...
tempDB = client.get_database('newevaltemplates')
...
def repeaPageGetSameSizeSummaryListsNew(pageSummary, pageName):
    # even a same URL repeats, it is possible that different contetn loads
    # hence identify possible different clusters for the same URL
    dictSummaryBySize = {}
    for summaryDict in pageSummary:
       try:
           dictSummaryBySize[pageName].append(summaryDict)
       except KeyError:
           dictSummaryBySize[pageName] = [summaryDict]
    return dictSummaryBySize
...
def analyzeRepeatedPages(summary,insertedSum,tempcollection):
    ...
    for page, pageSummary in summary.items():
        if pageSummary.__len__() > 0:
            ...
            pageSummaryClusters = repeaPageGetSameSizeSummaryListsNew(pageSummary, page)
            print(pageSummaryClusters)
            ### Find smmary similarities to find the templates
            for ckey, sumCluster in pageSummaryClusters.items():
                ############################### the below will not executed in the new EVAL file
                ...
                #     print('SUMMARY SUCCESSFULL: ' + str(page))
######################################################>>>>>>>>>>>>>>>>>>>>
def findTextInResultedTagsOfPagesAlteredContent(summary,pagecontent):
    textInPage = []
    for tag, values in summary[0].items():
        ...
        for val in values:
            ...
            if len(val['variables'])>0: ############  added the if condition to remove non-variable tags
                print(tagName)
...
def generateNoiseTemplatesForTheService(service_identifier,serviceObjectID,x,tempDB,authcollection):
    ...
    foldername = FOLDER_NAME +'/'+ FOLDER_NAME + '_'+ service_identifier
    ...
    try:
        ...
        for page, pageSummary in summary.items():
            tempcollection.update({'page': page}, {'summary': pageSummary, 'insertions': insertedSum}, upsert=True)
            print('SUMMARY SUCCESSFULL: ' + str(page))
        #########
        ...
        print(set(findTextInResultedTagsOfPagesAlteredContent(pageSummary,pagecontent1)))
        # if len(summary) > 0:
        #     print('analyze repeated pages' + str(tempcollection))
...
newlist = ['office_365_contacts','github']
threads = []
for record in selectedservices.find({}): ##///////////critical
   #continue
   for x in collection.find({'service_idnetifier': record['service_idnetifier']}):
# for record in ['tesco']: ##///////////critical
#    for x in collection.find({'service_idnetifier': record}):
        maxScrapedPagesCount = 0
        service_identifier = x['service_idnetifier']
        isConnected = True #x['connected']
        print(isConnected)
        serviceObjectID = x['_id']
        if isConnected:
            process = Thread(target=generateNoiseTemplatesForTheService,
                             args=[service_identifier, serviceObjectID, x, tempDB, collection])
            process.start()
